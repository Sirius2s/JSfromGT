...
exports.main_handler = async (event, context, callback) => {
    ...
    if (event["trigger_name"] == 'remote') {
        console.log('remote触发:', event["user_event"])
        ...
        for (let i = 0; i < links.length; i++) {
            try {
                const { body } = await got(`${links[i]}${event["user_event"]}.js`, {
                    timeout: 5000,
                    retry: 2
                })
                eval(body)
        ...
        const config_file = `${process.env.RUNTIME_CODE_ROOT}/config.json`
        ...
        const config_diy_file = `${process.env.RUNTIME_CODE_ROOT}/config_diy.json`
        // const config_diy_file = "code/config_diy.json"
        try {
    ...
    if (true) {
        ...
        for (const script of scripts) {
            const name = `${process.env.RUNTIME_CODE_ROOT}/` + script + '.js'
            const param_run = {}
